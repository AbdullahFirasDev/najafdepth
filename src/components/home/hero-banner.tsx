...
export function HeroBanner({ heroBanner, featuredArticle }: HeroBannerProps) {
  ...
  const description = isCustomBanner
    ? (heroBanner?.description ??
      "منصة ثقافية عربية للقراءة والتحرير والنشر.")
    : (featuredArticle.excerpt ??
      featuredArticle.subtitle ??
      "قراءة مختارة من هيئة التحرير.");
  const eyebrow = isCustomBanner
    ? (heroBanner?.eyebrow ?? "منصة ثقافية")
    : (featuredArticle.category?.name ?? "اختيار التحرير");
        ...
        >
          ...
          <div className="flex flex-wrap gap-3">
            ...
            {!isCustomBanner ? null : (
              ...
              >
                <Link href={`/articles/${featuredArticle.slug}`}>المقال المميز</Link>
              </Button>
            )}
          </div>
        </motion.div>
